...
**Licensing note on the tower CSV**: it traces to "Tower Telekomunikasi 50K," Kubu Raya
Regency's own Open Data portal (`opendata.kuburayakab.go.id`), supplied to this project via
the AGAIF bootcamp's Module 6 (AD1002) materials. The portal's own listing leaves its license
field blank, so this is publicly published government infrastructure data, not data under a
confirmed open license such as CC-BY. It's bundled here because it's a public download from a
