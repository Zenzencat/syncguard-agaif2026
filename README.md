...
**Licensing note on the tower CSV**: it traces to "TOWER TELEKOMUNIKASI 50K" on Satu Data
Indonesia, the national open-data portal
(<https://data.go.id/dataset/dataset/tower-telekomunikasi-50k>; publisher listed as Kabupaten
Kubu Raya), supplied to this project via the AGAIF bootcamp's Module 6 (AD1002) materials —
whose own notebook cites the same page ("The dataset is taken from Satu Data Indonesia"). The
page marks the dataset "Terbuka" (open) but shows **no explicit license field**, so this is
publicly published government infrastructure data, not data under a confirmed open license such
as CC-BY. It's bundled here because it's a public download from the national open-data portal,
not because redistribution rights have been formally confirmed — see
[`project_abstract.md`](project_abstract.md) §3 for the full framing.
